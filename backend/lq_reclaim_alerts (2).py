    ...
    for key, trade in list(trades.items()):
        ...
        for _, row in new_rows.iterrows():
            ...
            if low <= trade["current_stop_price"]:
                ...
                await _send_close_message(symbol, trade, outcome, total_return, exit_price, candle_time)
            ...
            if not trade["partial_taken"] and high >= trade["target_price"]:
                ...
                await _send_partial_message(symbol, trade)
...
async def _send_partial_message(symbol: str, trade: dict):
    text = (
...
async def _send_close_message(symbol: str, trade: dict, outcome: str, total_return: float,
                                exit_price: float, exit_time: int):
